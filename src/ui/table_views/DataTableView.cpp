...
void DataTableView::mousePressEvent(QMouseEvent *event)
{
...
void DataTableView::mouseReleaseEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
...
void DataTableView::mouseMoveEvent(QMouseEvent *event)
{
...
void DataTableView::SlotShowSelectionRow()
{
    for(int i = 0; i < m_SelectedRowList.count(); i++)
    {
...
void DataTableView::SlotChangeFilter()
{
    m_SelectedRowList.clear();
}

void DataTableView::InitHeaderInfo(QList<int> ShowHeaderList)
{
...
void DataTableView::ClearSelection()
{
    m_SelectedRowList.clear();
    m_iPressRow = -1;
    this->clearSelection();
...
QList<int> &DataTableView::GetSelectedRows()
{
    return m_SelectedRowList;
}

int DataTableView::GetCurrectSelectedRow()
{
    if(0 < m_SelectedRowList.count())
    {
...
void DataTableView::SelSameProRow(int iRow)
{
    int iNum = m_SelectedRowList.indexOf(iRow);
    if(iNum >= 0)
    {
...
QList<int> DataTableView::GetSelRowDesc()
{
...
bool DataTableView::_CmpRowMax(int iFstRow, int iScdRow)
{
    bool blMax = false;
    if(iFstRow > iScdRow)
    {
